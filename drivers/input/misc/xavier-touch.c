...
static int input_handler(struct platform_device *touch_dev,
  ...
  event = (data[0] & XAVIER_TOUCH_ID_EVENT_MASK) >> XAVIER_TOUCH_EVENT_SHIFT;
  ...
  case BTN_TOUCH_EVENT: /* btn_touch event */
    ...
    if (xavier_touch->btn_0 != btn0_value) {
      printk(KERN_INFO "Xavier : BTN_TOUCH_0 event\n");
      xavier_touch->btn_0 = btn0_value;
      input_report_key(xavier_touch->input_dev, xavier_touch->btn0, btn0_value);
    }

    if (xavier_touch->btn_1 != btn1_value) {
      printk(KERN_INFO "Xavier : BTN_TOUCH_1 event\n");
      xavier_touch->btn_1 = btn1_value;
      input_report_key(xavier_touch->input_dev, xavier_touch->btn1, btn1_value);
    }
  ...
  case START_RING_EVENT: /* start_ring event */
    ...
    printk(KERN_INFO "Xavier : START RING event\n");
  ...
  case END_RING_EVENT: /* end_ring */
    printk(KERN_INFO "Xavier : END RING event\n");
    location = ((data[0] & XAVIER_TOUCH_LOCATION_EVENT_MSB_MASK) <<
		XAVIER_TOUCH_LOCATION_EVENT_MSB_SHIFT);
    location += ((data[1] & XAVIER_TOUCH_LOCATION_EVENT_LSB_MASK) >>
		 XAVIER_TOUCH_LOCATION_EVENT_LSB_SHIFT);
  ...
  case POSITION_CHANGE_EVENT: /* position_change_ring */
    printk(KERN_INFO "Xavier : POSITION event\n");
    location = ((data[0] & XAVIER_TOUCH_LOCATION_EVENT_MSB_MASK) <<
		XAVIER_TOUCH_LOCATION_EVENT_MSB_SHIFT);
    location += ((data[1] & XAVIER_TOUCH_LOCATION_EVENT_LSB_MASK) >>
		 XAVIER_TOUCH_LOCATION_EVENT_LSB_SHIFT);
  ...
  case ERROR_EVENT: /* error event */
    printk(KERN_INFO "Xavier : Error event\n");
...
static int xavier_touch_probe(struct platform_device *pdev)
{
  ...
  printk(KERN_INFO "xavier :Xavier touch probe started\n");
  xavier_dev = dev_get_drvdata(pdev->dev.parent);

  xavier_touch = kzalloc(sizeof(struct xavier_touch), GFP_KERNEL);
  if (!xavier_touch) {
    ret = -ENOMEM;
    printk(KERN_ERR "Xavier : %s - Failed to allocate structure", __func__);
    goto error;
  }
  ...
  if (!input_dev) {
    ret = -ENOMEM;
    printk(KERN_ERR "Xavier : %s - Failed to allocate input device", __func__);
    goto error_free;
  }
  ...
  if (ret) {
    printk(KERN_ERR "Xavier : %s - Failed to register input device", __func__);
    goto error_free_device;
  }
  ret = sysfs_create_group(&xavier_touch->mfd->dev->kobj, &xavier_attr_group);
  if (ret) {
    printk(KERN_INFO "xavier : %s - Failed to create sysfs entries\n",__func__);
    goto error_free_device;
  }
