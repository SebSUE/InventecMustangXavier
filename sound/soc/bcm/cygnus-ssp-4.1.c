...
#define OUT_I2S_0_I2S_LEGACY_OFFSET   0xa18
...
#define IOP_PLL_0_USER_MDIV_CH0_OFFSET		0xb14
#define IOP_PLL_0_ACTIVE_NDIV_OFFSET			0xb20
#define IOP_PLL_0_ACTIVE_MDIV_CH0_OFFSET		0xb30
#define IOP_PLL_0_LDO_CTRL_OFFSET				0xb4c
#define IOP_PLL_0_LDO_PWRON_OFFSET			0xb50
#define IOP_PLL_0_LOCK_STATUS_OFFSET			0xb54
#define IOP_PLL_0_PWRON_OFFSET				0xb58
...
static int audio_ssp_init_portregs(struct cygnus_aio_port *aio)
{
	...
	case PORT_TDM:
printk("[ADK]\t\t%s entered, port TDM\n", __func__);
		value = readl(aio->cygaud->audio + aio->regs.i2s_stream_cfg);
		value &= ~I2S_STREAM_CFG_MASK;

printk("[ADK]\t\t%s: BF_CTRL_SOURCECH_GRPx(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.bf_sourcech_grp), aio->portnum);
		/* Set Group ID */
		writel(aio->portnum, aio->cygaud->audio + aio->regs.bf_sourcech_grp);
...
printk("[ADK]\t\t%s: FMM_IOP_OUT_I2S_x_STREAM_CFG(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.i2s_stream_cfg), value);
...
printk("[ADK]\t\t%s: FMM_BF_CTRL_SOURCECH_CFGX(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.bf_sourcech_cfg), value);
...
printk("[ADK]\t\t%s: FMM_IOP_IN_I2S_x_CAP_STREAM_CFG_0(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->i2s_in + aio->regs.i2s_cap_stream_cfg), value);
...
printk("[ADK]\t\t%s: FMM_BF_CTRL_DESTCH_CFGX_REG_BASE(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.bf_destch_cfg), value);
...
printk("[ADK]\t\t%s: MISC_SEROUT_OE_REG_BASE(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + AUD_MISC_SEROUT_OE_REG_BASE), value);
		writel(value, aio->cygaud->audio + AUD_MISC_SEROUT_OE_REG_BASE);
		break;
	case PORT_SPDIF:
printk("[ADK]\t\t%s entered, port SPDIF\n", __func__);
...
printk("[ADK]\t\t%s: FMM_IOP_MISC_IOP_SW_INIT_LOGIC(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->i2s_in + IOP_SW_INIT_LOGIC), value);
...
static void audio_ssp_in_reset_toggle(struct cygnus_aio_port *aio)
{
	u32 value;
...
printk("[ADK]\t\t%s entered, port %d\n", __func__, aio->portnum);

	value = readl(aio->cygaud->audio + aio->regs.bf_destch_cfg);
	value |= BIT(BF_DST_CFGX_CAP_ENA);
printk("[ADK]\t\t%s: FMM_BF_CTRL_DESTCH_CFGx_REG(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.bf_destch_cfg), value);
	writel(value, aio->cygaud->audio + aio->regs.bf_destch_cfg);
...
printk("[ADK]\t\t%s: FMM_IOP_OUT_I2S_x_I2S_CFG_REG(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.i2s_cfg), value);
	writel(value, aio->cygaud->audio + aio->regs.i2s_cfg);


	value = readl(aio->cygaud->i2s_in + aio->regs.i2s_cap_stream_cfg);
	value |= BIT(I2S_IN_STREAM_CFG_CAP_ENA);
printk("[ADK]\t\t%s: FMM_IOP_IN_I2S_x_CAP(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->i2s_in + aio->regs.i2s_cap_stream_cfg), value);
	writel(value, aio->cygaud->i2s_in + aio->regs.i2s_cap_stream_cfg);


		value = 0x1;
printk("[ADK]\t\t%s: FMM_BF_CTRL_DESTCH_CTRLx(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.bf_destch_ctrl), value);
	writel(value, aio->cygaud->audio + aio->regs.bf_destch_ctrl);


	/* Enable input portion of block */
	/* IOP SW INIT on IN_I2S_x */
 	audio_ssp_in_reset_toggle(aio);
#if 0
	udelay(10);
	value = readl(aio->cygaud->i2s_in + IOP_SW_INIT_LOGIC);
	value |= BIT(IOP_LOGIC_RESET_IN_OFFSET(aio->portnum));
...
printk("[ADK]\t\t%s: FMM_IOP_OUT_I2S_x_STREAM_CFG(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.i2s_stream_cfg), value);
		...
		value |= BIT(I2S_OUT_CFGX_CLK_ENA);
		value |= BIT(I2S_OUT_CFGX_DATA_ENABLE);
printk("[ADK]\t\t%s: FMM_IOP_OUT_I2S_x_I2S_CFG(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.i2s_cfg), value);
		writel(value, aio->cygaud->audio + aio->regs.i2s_cfg);
...
printk("[ADK]\t\t%s entered, active_ports=%d, rate=%ld\n", __func__, cygaud->active_ports, vco_rate);
...
printk("[ADK]\t%s clk_set_rate(): pll_ch_num=%d returns %d\n", __func__, p_entry->pll_ch_num, error);
...
printk("[ADK] %s: FMM_IOP_OUT_I2S_x_MCLK_CFG_0(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.i2s_mclk_cfg), value);
...
printk("[ADK] %s entered, port=%d, mode=%d, params_channels=%d\n", __func__, aio->portnum, aio->mode, params_channels(params));
...
printk("[ADK] %s stream=CAPTURE\n", __func__);
		switch (params_format(params)) {
		case SNDRV_PCM_FORMAT_S16_LE:
printk("[ADK] %s format=S16_LE\n", __func__);
			value = readl(aio->cygaud->audio +
	...
	if (!aio->is_slave) {
printk("[ADK] %s call cygnus_ssp_set_clocks(), rate=%d\n", __func__, rate);
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	printk("[ADK] %s Enter port = %d, freq=%d\n", __func__, aio->portnum, freq);
...
printk("[ADK] %s: Setting MCLKSEL to %d\n", __func__, sel);
	value = readl(aio->cygaud->audio + aio->regs.i2s_mclk_cfg);
...
printk("[ADK] %s entered, fmt: %x, port=%d\n", __func__, fmt, aio->portnum);
...
printk("[ADK] %s entered, fmt: %x, is_slave=%d, width=%d\n", __func__, fmt, aio->is_slave, aio->fsync_width);
...
printk("[ADK] %s entered, fmt: %x, mode=%d\n", __func__, fmt, aio->mode);
...
printk("[ADK] %s: FMM_IOP_OUT_I2S_x_I2S_CFG(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + aio->regs.i2s_cfg), ssp_outcfg);
...
printk("[ADK] %s: curcfg=0x%08x, newcfg=0x%08x\n", __func__, ssp_curcfg, ssp_newcfg);
//	ssp_incfg = (ssp_curcfg & I2S_IN_CFG_REG_UPDATE_MASK) | ssp_newcfg;
	ssp_incfg = (ssp_curcfg | ssp_newcfg) & 0xc003fff0;
printk("[ADK] %s: FMM_IOP_IN_I2S_x_I2S_CFG(%p):=0x%08x, mask=0x%08x\n", __func__, (void *)(aio->cygaud->i2s_in + aio->regs.i2s_cap_cfg), ssp_incfg, I2S_IN_CFG_REG_UPDATE_MASK);
...
printk("[ADK]\t\t%s: MISC_SEROUT_OE_REG_BASE(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->audio + AUD_MISC_SEROUT_OE_REG_BASE), val);
	writel(val, aio->cygaud->audio + AUD_MISC_SEROUT_OE_REG_BASE);

/* [ADK] add this ..Not need?
	val = 0x1;
	writel(val, aio->cygaud->audio + OUT_I2S_0_I2S_LEGACY_OFFSET);
----------------- */

printk("[ADK]\t\t%s: IOP_PLL_0_CONTROL			=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_CONTROL_OFFSET			));
printk("[ADK]\t\t%s: IOP_PLL_0_USER_NDIV		=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_USER_NDIV_OFFSET			));
printk("[ADK]\t\t%s: IOP_PLL_0_USER_MDIV_CH0	=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_USER_MDIV_CH0_OFFSET		));
printk("[ADK]\t\t%s: IOP_PLL_0_ACTIVE_NDIV		=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_ACTIVE_NDIV_OFFSET		));
printk("[ADK]\t\t%s: IOP_PLL_0_ACTIVE_MDIV_CH0	=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_ACTIVE_MDIV_CH0_OFFSET	));
printk("[ADK]\t\t%s: IOP_PLL_0_LDO_CTRL		=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_LDO_CTRL_OFFSET			));
printk("[ADK]\t\t%s: IOP_PLL_0_LDO_PWRON		=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_LDO_PWRON_OFFSET		));
printk("[ADK]\t\t%s: IOP_PLL_0_LOCK_STATUS		=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_LOCK_STATUS_OFFSET		));
printk("[ADK]\t\t%s: IOP_PLL_0_PWRON			=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_PWRON_OFFSET				));
printk("[ADK]\t\t%s: IOP_PLL_0_RESET			=0x%08x\n", __func__, readl(aio->cygaud->audio + IOP_PLL_0_RESET_OFFSET				));
	...
	unsigned int tx_mask, unsigned int rx_mask, int slots, int slot_width)
{
	...
	printk("[ADK] %s entered, mask(tx_,rx_)=(0x%08x, 0x%08x), slots=%d, slot_width=%d\n", __func__, tx_mask, rx_mask, slots, slot_width);
...
*/
	...
	printk("[ADK] %s active_slots %u, bits per frame %d\n", __func__, active_slots, frame_bits);

/*
printk("[ADK] %s i2s_in@%p\n", __func__, (void *)(aio->cygaud->i2s_in));
printk("[ADK] %s i2s_cap_cfg@%p\n", __func__, (void *)(aio->regs.i2s_cap_cfg));
printk("[ADK] %s i2s_cfg@%p\n", __func__, (void *)(aio->regs.i2s_cfg));
*/
...
printk("[ADK] %s: FMM_IOP_IN_I2S_x_I2S_CFG(%p):=0x%08x\n", __func__, (void *)(aio->cygaud->i2s_in + aio->regs.i2s_cap_cfg), value);
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = pll_fract_tweak_get(aio->cygaud->audio);
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	pll_fract_tweak_set(aio->cygaud->audio, value);
...
printk("[ADK]\t%s/%d parent@%p\n", __func__, __LINE__, (void *)parent);
...
printk("[ADK] %s aud=0x%08x\n", __func__, (u32)cygaud->audio);
...
printk("[ADK] %s i2s_in=0x%08x\n", __func__, (u32)cygaud->i2s_in);

	/* Tri-state all controlable pins until we know that we need them */
/*  [ADK]	12/18/2015 keep output enable for debugging ..
...
printk("[ADK] %s Registering %d DAIs\n", __func__, active_port_count);
