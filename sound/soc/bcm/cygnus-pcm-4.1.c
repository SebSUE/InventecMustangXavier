...
printk("[ADK] %s entered, start=0x%08x, bufsize=0x%08x, rbuf@%p\n", __func__, start, bufsize, p_rbuf);
...
printk("[ADK] %s aio@%p, p_rbuf@%p\n", __func__, audio_io, p_rbuf);
...
printk("[ADK]\t%s entered, [%d]: port=%d\n", __func__, substream->stream, aio->portnum);
...
if (dbg_irqs < 8) {
	int idx, cnt;
	char *pb = runtime->dma_area;
	
	unsigned int offset = readl(aio->cygaud->audio + p_rbuf->rdaddr);
	unsigned int base = readl(aio->cygaud->audio + p_rbuf->baseaddr);

	printk("[ADK]\trdaddr=0x%08x, base@(pa=0x%08x, va=%p)\n", offset, base, pb);
	offset &= 0xffff;
	pb += offset;
//	printk("[ADK] offset=0x%08x, base=0x%08x, dma: %zu bytes@%p\n", offset, base, runtime->dma_bytes, pb);

	for (idx=0, cnt=0; idx < 1024; idx++) {
		if (*pb) cnt++;
		pb++;
	}
	printk("[ADK] find a non-zero bytes: %d\n", cnt);
	dbg_irqs++;
}
...
if (dbg_irqs < 8) {
	printk("[ADK]\tsetup rdaddr:=0x%08x\n", regval);
}
	}
}
...
if (dbg_irqs < 8) {
	printk("[ADK] %s: esr_status2=0x%08x, esr_status4=0x%08x\n", __func__, esr_status2, esr_status4);
}
...
static int cygnus_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	printk("[ADK] %s (buf_size %lu) (period_size %lu), start@(va=%p, pa=0x%08x)\n", __func__, bufsize, periodsize, runtime->dma_area, runtime->dma_addr);
	printk("[ADK] %s dma: start@(va=%p, pa=0x%08x)\n", __func__, substream->dma_buffer.area, substream->dma_buffer.addr);
...
static int cygnus_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	printk("[ADK] %s entered, DMA buf@%p\n", __func__, buf);
	...
	printk("[ADK] %s: [%d]: size 0x%zx @ (va=%p, pa=0x%08x)\n", __func__, stream, size, buf->area, buf->addr);
...
static int cygnus_dma_new(struct snd_soc_pcm_runtime *rtd)
{
	...
	printk("[ADK] %s Enter, card@%p, pcm@%p\n", __func__, card, pcm);
