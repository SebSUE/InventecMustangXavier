...
#ifdef CONFIG_BCM_CSX_CYGNUSAUDIO
#include <linux/csx_cygnusaudio.h>
#endif
...
/*
 * Enable __DIAGNOSTICS__ to debug the time intervals when each
 * playback interrupt happens. This is for diagnostics purposes
 */
#undef __DIAGNOSTICS__
...
/* CSX Data stucture */
struct pcm_csx_data {
	struct csx_io_point_fncs    csx_ops;
	void                *priv;
};

struct cygnusaudio_csx_framesync {
	CSX_FRAME_SYNC_FP frame_sync_fp;
	void     *data;
};

/* CSX data */
/* Array of CSX data structures */
static struct pcm_csx_data   csx_data[CYGNUSAUDIO_NUM_CSX_POINTS];

/* CSX frame sync callback */
static struct cygnusaudio_csx_framesync g_csx_frame_sync;
#endif
...
static void cygnus_pcm_period_elapsed(struct snd_pcm_substream *substream)
{
	...
	if (g_csx_frame_sync.frame_sync_fp != NULL)
		g_csx_frame_sync.frame_sync_fp(g_csx_frame_sync.data);
#endif
	/*
...
#ifdef CONFIG_BCM_CSX_CYGNUSAUDIO
static char *previous_hwbuf = 0x0;
#endif
...
#ifdef CONFIG_BCM_CSX_CYGNUSAUDIO
	if ((substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		&& (csx_data[CYGNUSAUDIO_CSX_POINT_DAC].csx_ops.csx_callback)) {
		if (res == 0x0)
			hwbuf = runtime->dma_area + p_rbuf->buf_size
				- p_rbuf->period_bytes;
		else
			hwbuf = runtime->dma_area + res - p_rbuf->period_bytes;

		if (previous_hwbuf != hwbuf)
			csx_data[CYGNUSAUDIO_CSX_POINT_DAC].csx_ops
			.csx_callback((char *)hwbuf, p_rbuf->period_bytes,
			csx_data[CYGNUSAUDIO_CSX_POINT_DAC].priv);

		previous_hwbuf = hwbuf;
	} else if ((substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		&& (csx_data[CYGNUSAUDIO_CSX_POINT_ADC].csx_ops.csx_callback)) {
		if (res == 0x0)
			hwbuf = runtime->dma_area + p_rbuf->buf_size
				- p_rbuf->period_bytes;
		else
		...
		if (previous_hwbuf != hwbuf)
			csx_data[CYGNUSAUDIO_CSX_POINT_ADC].csx_ops
			.csx_callback((char *)hwbuf, p_rbuf->period_bytes,
			csx_data[CYGNUSAUDIO_CSX_POINT_ADC].priv);
...
*/
int cygnus_audio_set_csx_io_points(
	unsigned int           cid,    /**< (i) Codec to install CSX
					      *       callbacks
					      */
	int    point,  /**< (i) Point ID for CSX
			*       callbacks
			*/
	const struct csx_io_point_fncs *fncp,   /**< (i) List of callbacks */
...
)
{
	...
	switch (point) {
	case CYGNUSAUDIO_CSX_POINT_ADC:
	{
		memcpy(&(csx_data[CYGNUSAUDIO_CSX_POINT_ADC].csx_ops),
			fncp,
			sizeof(csx_data[CYGNUSAUDIO_CSX_POINT_ADC].csx_ops));

		csx_data[CYGNUSAUDIO_CSX_POINT_ADC].priv = data;
		break;
	}
	case CYGNUSAUDIO_CSX_POINT_DAC:
	{
		memcpy(&(csx_data[CYGNUSAUDIO_CSX_POINT_DAC].csx_ops),
			fncp,
			sizeof(csx_data[CYGNUSAUDIO_CSX_POINT_DAC].csx_ops));

		csx_data[CYGNUSAUDIO_CSX_POINT_DAC].priv = data;
		break;
	}
	default:
		err = -EINVAL;
...
EXPORT_SYMBOL(cygnus_audio_set_csx_io_points);
...
*     -ve      Other error codes
*/
int cygnus_audio_set_csx_frame_sync(
	CSX_FRAME_SYNC_FP fncp,    /**< (i) Frame sync callback */
	void              *data    /**< (i) User data */
)
{
	memset(&g_csx_frame_sync, 0, sizeof(struct cygnusaudio_csx_framesync));
	memset(csx_data, 0, sizeof(struct pcm_csx_data));

	if (fncp != NULL) {
		g_csx_frame_sync.frame_sync_fp = fncp;
		g_csx_frame_sync.data = data;
	} else {
		g_csx_frame_sync.frame_sync_fp = NULL;
		g_csx_frame_sync.data = NULL;
	}

	return 0;
}
EXPORT_SYMBOL(cygnus_audio_set_csx_frame_sync);
#endif
